<template>
  <v-app>
    <SideBar />
    <div class="content">
      <h2 class="header mb-2">Dashboard</h2>
      <v-container fluid>
        <v-row dense>
          <v-col>
            <v-card color="white" class="rounded-lg">
              <v-list>
                <v-list-item>
                  <v-list-item-icon>
                    <v-icon color="primary" large> mdi-server</v-icon>
                  </v-list-item-icon>
          ...
          <v-col>
            <v-card color="white" class="rounded-lg">
              <v-list>
                <v-list-item>
                  <v-list-item-icon>
                    <v-icon color="primary" large> mdi-folder-multiple</v-icon>
                  </v-list-item-icon>
        ...
        <v-row>
          ...
          <v-col cols="12" sm="4">
            <v-card style="height: 50vh">
              <v-subheader>Monitoring Type</v-subheader>
        ...
        <v-row>
          <v-col>
            <v-card>
              ...
              <v-data-table :headers="headers" :items="monitoringData">
                <template v-slot:top>
                  <v-toolbar flat>
                    <v-toolbar-title class="mr-15"
                      >Monitoring Management</v-toolbar-title
                    >
                ...
                <template v-slot:item.actions="{ item }">
                  <v-icon small class="mr-2" @click="viewALlData(item)"
                    >mdi-mail
                  </v-icon>
                </template>
              </v-data-table>
...
export default {
  ...
  methods: {
    ...
    getChartProject() {
      axios
        .get("http://localhost:8080/project/count")
        .then((res) => {
    ...
    getChartServer() {
      axios
        .get("http://localhost:8080/server/count")
        .then((res) => {
    ...
    getChartService() {
      axios
        .get("http://localhost:8080/monitoring/countService")
        .then((res) => {
    ...
    getChartEngine() {
      axios
        .get("http://localhost:8080/monitoring/countEngine")
        .then((res) => {
